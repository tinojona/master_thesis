...
for(i in station_names){
  subset = data[data$station == i,]
  n = max(subset$all)
  hist(subset$all, breaks = n+1, main = i, xlab = "daily hospitalizations")
}
...
print(paste0( "With this adjustement, we still have ", sum(data_new$all), " hospitalizations in the analysis from ", nrow(data_new), " observations, which equals ", (sum(data_new$all)/nrow(data_new)) , " hospitalizations per day on average"))
```
