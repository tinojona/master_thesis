...
# - the lower the binary threshold the better the model performance
...
bin_thresholds = c(5,10,20,40,80,120,160, 180 ,200,240,270)
...
mod_w_modifier <- gnm(all ~  cb.temp + modif, data = data,  family=quasipoisson(), eliminate=stratum_dow, subset=ind_dow>0)

# prediction with and without foehn
pred <- crosspred(cb.temp, mod, cen = 28, cumul=FALSE)
pred_w_modifier <- crosspred(cb.temp, mod_w_modifier, cen = 28, cumul=FALSE)




plot(pred,              ## cumulative exposure
     "overall",
     col = 2,
...
lines(pred_w_modifier,           ## cumulative exposure
     "overall",
     col = 4,
     ci = "area",
...
legend("topright", legend = c("temp", "temp+modifier foehn"), col = c(2,4), lwd = 2)
}
#####
